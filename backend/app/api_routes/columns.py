...
async def update_column(column_id: uuid.UUID, body: ColumnUpdate, db: AsyncSession = Depends(get_db)):
    return await ColumnService(db).update(column_id, body)

@router.delete('/{column_id}', status_code=status.HTTP_204_NO_CONTENT)
async def delete_column(column_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
