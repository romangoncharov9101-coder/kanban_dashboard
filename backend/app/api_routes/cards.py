...
async def created_card(body: CardCreate, db: AsyncSession = Depends(get_db)):
    return await CardService(db).create(body)

@router.put('/{card_id}', response_model=CardOut)
async def update_card(card_id: uuid.UUID, body: CardUpdate, db: AsyncSession = Depends(get_db)):
...
async def move_card(card_id: uuid.UUID, body: CardMoveRequest, db: AsyncSession = Depends(get_db)):
