...
async def list_cards(
    ...
    return await CardService(db).get_all(
        viewer=current_user, column_id=column_id, assigned_to=assigned_to,
...
async def update_card(card_id: uuid.UUID, body: CardUpdate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    """Право проверяется по авторству задачи внутри сервиса."""
    return await CardService(db).update(card_id, body, current_user)
...
async def move_card(card_id: uuid.UUID, body: CardMoveRequest, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    """Доступно всем ролям; для USER проверяется исполнительство и разрешённая категория."""
    return await CardService(db).move(card_id, body, current_user)
...
async def upload_attachment(card_id: uuid.UUID, file: UploadFile, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    return await CardService(db).upload_card_file(card_id, file, current_user)
...
async def download_attachment(
    ...
    response = await CardService(db).get_attachment_file_response(attachment_id, current_user)
...
async def delete_attachment(attachment_id: uuid.UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    return await CardService(db).delete_attachment(attachment_id, current_user)


@router.get('/{card_id}/comments', response_model=list[CommentOut])
async def list_comments(card_id: uuid.UUID, last_id: uuid.UUID | None = Query(None, description='ID для пагинации'), db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    return await CardService(db).get_comments(card_id, current_user, last_id)
...
async def add_comment(card_id: uuid.UUID, body: CommentCreate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    return await CardService(db).add_comment(card_id=card_id, actor=current_user, text=body.text)
...
async def delete_comment(comment_id: uuid.UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    return await CardService(db).delete_comments(comment_id, current_user)
...
async def archive_card(card_id: uuid.UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    return await CardService(db).archive(card_id, current_user)
...
async def unarchive_card(card_id: uuid.UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)):
    return await CardService(db).unarchive(card_id, current_user)
